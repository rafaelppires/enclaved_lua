...
static int l_sgx_process(lua_State *L) {
    ...
    char buff[BUFSIZ];
    ecall_execfunc(global_eid, &result, code, csz, data, dsz, buff,
                   sizeof(buff));
...
static int l_sgx_encrypt(lua_State *L) {
    ...
    Crypto::encrypt_aes_inline("key", cipher);
...
static int l_sgx_decrypt(lua_State *L) {
    ...
    Crypto::decrypt_aes_inline("key", plain);
...
//---------- begin of includes for epc usage drive query
#include <sys/ioctl.h>
#include <isgx_user.h>
#include <fcntl.h>
#include <stdio.h>
#include <unistd.h>
//---------- end of includes for epc usage drive query
...
int main(int argc, char **argv) {
    ...
    //---------- begin of epc usage drive query
    ...
    struct sgx_enclave_usage arg;
    arg.sgx_pid = getpid();
    printf("cmd = %lu pid = %llu\n", SGX_IOC_EPC_USAGE, arg.sgx_pid);

    int ret = ioctl(fd, SGX_IOC_EPC_USAGE, &arg);
    if (ret) {
        perror("damn ");
        printf("%d\n", ret);
    } else
        printf(
            "ret: %d enclave_cnt:%llu epc_pages_cnt:%llu va_pages_cnt:%llu\n",
            ret, arg.enclave_cnt, arg.epc_pages_cnt, arg.va_pages_cnt);
    //---------- end of epc usage drive query
